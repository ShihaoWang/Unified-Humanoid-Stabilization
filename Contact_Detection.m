function  [position,isterminal,direction] = Contact_Detection(t,z,P)

% This function is used to detect the current mode status of the robot.
% The mode status will get updated if there is a removal or addition of the
% contact. 
...
% The main events can be divided into two parts: adding contact or removing
% contact

% For adding contact, there are two possibilities: making a contact or the
% velocity at end effector decreases to zero.

% For removing contact, there are one possibility: the normal force at the
% contact point turns into negative

% For a given point, the adding dynamics case: 
% 1. Adding contact due to impact,
% 2. Sliding end effector declines to sticking.

% While, the removing dynamics case:
% 1. Normal contact force becomes zero,
% 2. Sticking end effector begins to slide.

[position_A_i, isterminal_A_i, direction_A_i] = Event_Condition_Generation(contact_status, 1, Contact_Force_AtoO, P);
[position_B_i, isterminal_B_i, direction_B_i] = Event_Condition_Generation(contact_status, 2, Contact_Force_AtoO, P);
[position_C_i, isterminal_C_i, direction_C_i] = Event_Condition_Generation(contact_status, 3, Contact_Force_AtoO, P);
[position_D_i, isterminal_D_i, direction_D_i] = Event_Condition_Generation(contact_status, 4, Contact_Force_AtoO, P);
[position_M_i, isterminal_M_i, direction_M_i] = Event_Condition_Generation(contact_status, 5, Contact_Force_AtoO, P);
[position_O_i, isterminal_O_i, direction_O_i] = Event_Condition_Generation(contact_status, 6, Contact_Force_AtoO, P);

% The current case is the point sticking to the ground, so the
% potential change to the dynamics is the contact force case
position =   [  position_A_i;    position_B_i;     position_C_i;    position_D_i;     position_M_i;   position_O_i];
isterminal = [isterminal_A_i;  isterminal_B_i;   isterminal_C_i;  isterminal_D_i;   isterminal_M_i; isterminal_O_i];  % Halt integration
direction =  [ direction_A_i;   direction_B_i;    direction_C_i;   direction_D_i;    direction_M_i;  direction_O_i];

events_state = [events_state; position_A_i([1,4])',position_B_i([1,4])',position_C_i([1,4])',position_D_i([1,4])',position_M_i([1,4])',position_O_i([1,4])'];
